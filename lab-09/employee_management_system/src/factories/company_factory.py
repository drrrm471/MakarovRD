"""Фабрика для создания компаний (Abstract Factory паттерн)."""
...
class CompanyFactory(ABC):
    ...
    @abstractmethod
    def _create_project(
        self, project_id: int, name: str, description: str, deadline: str
    ) -> Project:
        """Создать проект."""
        pass
...
class SalesCompanyFactory(CompanyFactory):
    ...
    def create_company(self, name: str) -> Company:
        ...
        vending_project = self._create_project(
            project_id=201, name="Vending", description="Вендинг", deadline="2024-12-31"
        )
